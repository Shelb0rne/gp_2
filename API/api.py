...
selenium_data = pd.read_csv('selenium_cars_final.csv')
data = data.dropna(subset=['year', 'make', 'model', 'url'])
selenium_data = selenium_data.dropna(subset=['year', 'make', 'model', 'url'])
year_make_model = data[['year', 'make', 'model', 'url']]
selenium_year_make_model = selenium_data[['year', 'make', 'model', 'url']]
new_data = []
selenium_new_data = []
print(API_KEY)
...
for i in tqdm(range(len(selenium_year_make_model))):
    year = selenium_year_make_model.iloc[i]['year']
    make = str(selenium_year_make_model.iloc[i]['make'])
    model = str(selenium_year_make_model.iloc[i]['model'])
    url = selenium_year_make_model.iloc[i]['url']
    info = getData(make, model, int(year), url)

    selenium_new_data.append(info)
    time.sleep(0.3)
    if i % 50 == 0:
        pd.DataFrame(selenium_new_data).to_csv('selenium_checkpoint.csv', index= False)
